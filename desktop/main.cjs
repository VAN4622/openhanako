...
// 单实例：避免用户重复启动时出现多个 Electron 前端同时连同一个后台 server。
...
function monitorServer() {
  if (serverMode !== "local" || !serverProcess) return;
...
function createMainWindow() {
  ...
  // Dev 模式优先走 Vite dev server；如果没起 Vite，则回退到已构建产物。
  // 源码版 index.html 会直接引用 main.tsx，在 file:// 下无法作为模块脚本运行。
...
function createSettingsWindow(tab, theme) {
  ...
  // Dev 模式优先走 Vite dev server；如果没起 Vite，则回退到已构建产物。
  // 源码版 settings.html 会直接引用 settings-main.tsx，在 file:// 下无法作为模块脚本运行。
  const isDev = process.argv.includes("--dev");
...
app.whenReady().then(async () => {
  try {
    ...
    if (gateway.mode === "remote") {
      ...
      console.log("[desktop] 鍚姩 Hanako Server...");
    }
...
app.on("second-instance", () => {
  console.log("[desktop] 检测到重复启动，唤起现有窗口");
  if (mainWindow && !mainWindow.isDestroyed()) {
    if (mainWindow.isMinimized()) mainWindow.restore();
    mainWindow.show();
    mainWindow.focus();
