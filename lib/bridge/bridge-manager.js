...
import { createWeixinAdapter, DEFAULT_WEIXIN_BASE_URL } from "./weixin-adapter.js";
...
const ADAPTER_REGISTRY = {
  ...
  weixin: {
    create: (creds, onMessage, hooks) => createWeixinAdapter({
      baseUrl: creds.baseUrl || DEFAULT_WEIXIN_BASE_URL,
      token: creds.token,
      accountId: creds.accountId,
      userId: creds.userId,
      storageDir: hooks?.storageDir,
      cdnBaseUrl: creds.cdnBaseUrl,
      onMessage,
      onStatus: hooks?.onStatus,
    }),
    getCredentials: (cfg) => cfg?.enabled && cfg?.token
      ? {
          baseUrl: cfg.baseUrl,
          token: cfg.token,
...
export class BridgeManager {
  ...
  startPlatform(platform, credentials) {
    ...
    try {
      ...
      const hooks = {
        ...
        storageDir: path.join(this.engine.agentDir, "bridge-media", platform),
  ...
  async _handleMessage(platform, msg) {
    const { sessionKey, text, displayText, images, senderName, avatarUrl, userId, isGroup, chatId } = msg;
    const entry = this._platforms.get(platform);
    ...
    this._pushMessage({
      ...
      sender: senderName || "用户", text: displayText || text,
      isGroup, ts: Date.now(),
    });
    ...
    if (isGroup) {
      ...
      this._flushGroupMessage(platform, chatId, sessionKey, line, meta, images);
      return;
    }
    ...
    if (Array.isArray(images) && images.length > 0) pending.images.push(...images);
   ...
   */
  async _flushGroupMessage(platform, chatId, sessionKey, line, meta, images = []) {
    const entry = this._platforms.get(platform);
    if (!entry?.adapter) return;
  ...
  async _flushPending(sessionKey) {
    const pending = this._pending.get(sessionKey);
    if (!pending || (pending.lines.length === 0 && (!pending.images || pending.images.length === 0))) return;
    ...
    const images = Array.isArray(pending.images) ? pending.images.splice(0) : [];
    const { platform, chatId, senderName, avatarUrl, userId, isGroup, isOwner, agentId } = pending;
    this._pending.delete(sessionKey);

    const mergedBody = lines.join("\n").trim();
    const merged = mergedBody ? `${timeTag()} ${mergedBody}` : `${timeTag()} [图片]`;
    const meta = { name: senderName, avatarUrl, userId };
    ...
    adapter?.startTyping?.(chatId).catch(() => {});
    ...
    try {
      ...
      if (adapter) {
        const cleaned = this._cleanReplyForPlatform(reply || "");
        const mediaFiles = this.engine.consumeBridgePresentedFiles?.(sessionKey) || [];
        if (!cleaned && mediaFiles.length === 0) {
          return;
        }

        if (mediaFiles.length > 0 && adapter?.sendMediaFiles) {
          await adapter.sendMediaFiles(chatId, mediaFiles, cleaned);
        } else if (useBlockStream && chunker) {
          // flush 剩余 buffer
        ...
        const mediaSummary = mediaFiles.length > 0
          ? mediaFiles.map((fp) => `[附件] ${path.basename(fp)}`).join("\n")
          : "";
        const displayText = [cleaned, mediaSummary].filter(Boolean).join("\n");
        this._pushMessage({
          platform, direction: "out", sessionKey,
          sender: this.engine.agentName, text: displayText,
          isGroup, ts: Date.now(),
        });
      }
    } catch (err) {
