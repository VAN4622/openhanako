...
export class BrowserManager {
  ...
  static setTransport(transport) {
    const changed = _transport !== transport;
    _transport = transport || null;
    if (changed && _instance && !process.send) {
  ...
  _sendCmd(cmd, params = {}, timeoutMs = 30000) {
    if (!process.send && _transport?.sendCmd) {
      return _transport.sendCmd(cmd, params, timeoutMs);
    }
    if (!process.send) {
      throw new Error("浏览器功能仅在 Electron 桌面模式下可用");
    }
  ...
  isTransportAvailable() {
    return Boolean(process.send || _transport?.sendCmd);
  }
}
