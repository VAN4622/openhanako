...
export class ConversationManager {
  ...
  async resetLocalSessionConversation(sessionPath, meta = {}) {
    if (!sessionPath) return null;
    return this._enqueue(() => this._resetBinding({
      bucket: "localSessions",
      sourceType: "local_session",
      sourceKey: this._localKey(sessionPath),
      sessionPath,
  ...
  async resetBridgeSessionConversation(sessionKey, { guest = false, meta = {} } = {}) {
    if (!sessionKey) return null;
    return this._enqueue(() => this._resetBinding({
      bucket: guest ? "bridgeGuestSessions" : "bridgeOwnerSessions",
      sourceType: guest ? "bridge_guest" : "bridge_owner",
  ...
  _linkBinding({ bucket, sourceType, sourceKey, sessionPath, conversationId, meta }) {
    const bindings = this._readBindings();
    const index = this._readIndex();
    const previous = normalizeBindingEntry(bindings[bucket]?.[sourceKey]);
    ...
    if (previous?.conversationId && previous.conversationId !== conversationId) {
      this._detachBindingFromConversation(index, previous.conversationId, bucket, sourceKey);
    }
    bindings[bucket][sourceKey] = {
      conversationId,
      lastSeenSeq: 0,
  ...
  _resetBinding({ bucket, sourceType, sourceKey, sessionPath, meta }) {
    const bindings = this._readBindings();
    const index = this._readIndex();
    const previous = normalizeBindingEntry(bindings[bucket]?.[sourceKey]);
    const conversationId = crypto.randomUUID();
    ...
    this._touchConversation(index, conversationId, {
      ...
      meta: {
        ...(meta || {}),
        resetFromConversationId: previous?.conversationId || null,
      },
      createdAt: nowIso(),
    });
    this._writeBindings(bindings);
    this._writeIndex(index);
    log.log(
      `reset ${sourceLabel(sourceType, sourceKey)} ${shortId(previous?.conversationId)} -> ${shortId(conversationId)}`,
    );
    return conversationId;
  }
  ...
  _detachBindingFromConversation(index, conversationId, bucket, sourceKey) {
    if (!conversationId || !bucket || !sourceKey) return;
    const current = index.conversations?.[conversationId];
    if (!current?.bindings?.[bucket]) return;
    current.bindings[bucket] = current.bindings[bucket].filter((item) => item !== sourceKey);
    current.updatedAt = nowIso();
    index.conversations[conversationId] = current;
  }
