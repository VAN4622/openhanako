...
describe("ConversationManager", () => {
  ...
  it("resets a bridge session into a new shared conversation without deleting old history", async () => {
    const { manager } = makeManager();

    const firstConversationId = await manager.appendBridgeSessionMessages("qq_dm_reset", [
      { role: "user", content: [{ type: "text", text: "old bridge history" }] },
    ]);

    const oldTimeline = manager.readTimeline(firstConversationId);
    expect(oldTimeline).toHaveLength(1);

    const nextConversationId = await manager.resetBridgeSessionConversation("qq_dm_reset");
    expect(nextConversationId).not.toBe(firstConversationId);
    expect(manager.getConversationIdForBridgeSession("qq_dm_reset")).toBe(nextConversationId);

    const oldBindingInfo = manager.readSnapshot(firstConversationId);
    expect(oldBindingInfo?.conversationId).toBe(firstConversationId);
    expect(manager.readTimeline(firstConversationId)).toHaveLength(1);
    expect(manager.readTimeline(nextConversationId)).toHaveLength(0);

    await manager.appendBridgeSessionMessages("qq_dm_reset", [
      { role: "user", content: [{ type: "text", text: "new bridge history" }] },
    ]);

    expect(manager.readTimeline(firstConversationId)).toHaveLength(1);
    expect(manager.readTimeline(nextConversationId)).toHaveLength(1);
    expect(manager.readTimeline(nextConversationId)[0].text).toBe("new bridge history");
  });
});
